...
def run_once_scan(
    ...
    *,
    ...
    for zip_path in filtered_paths:
        ...
        # Ki?m tra xem file dã du?c x? lý thành công chua
        already_processed, logged_mtime = has_success_log(zip_name, pg_cfg, ingest_schema=ingest_schema)
        
        if already_processed:
            # So sánh modification time - n?u KHÁC ? file dã du?c c?p nh?t ? re-ingest
            ...
            if current_mtime != logged_mtime:
                ...
                # Ti?p t?c x? lý (không continue)
            else:
                # Timestamp GI?NG NHAU ? skip
                logger.info(
