# modeling/logging_config.py
"""
Logging configuration for modeling pipeline.
...
def configure_logging(
    logs_dir: str = None,
    app_name: str = "modeling",
    level: int = logging.INFO,
) -> None:
    """
    Configure logging for the modeling pipeline.

    Args:
        logs_dir: Directory to store log files. Defaults to env var LOG_DIR or /logs/modeling
        app_name: Application name for log files
        level: Logging level (DEBUG, INFO, WARNING, ERROR, CRITICAL)
    """
    if logs_dir is None:
        logs_dir = os.getenv("LOG_DIR", "/logs/modeling")

    logs_path = Path(logs_dir)
    logs_path.mkdir(parents=True, exist_ok=True)

    # Root logger configuration
    root_logger = logging.getLogger("modeling")
...
def get_logger(name: str) -> logging.Logger:
    ...
    """
    return logging.getLogger(f"modeling.{name}")


# Initialize logging when module is imported
if not logging.getLogger("modeling").handlers:
    configure_logging()
